...
import type { Env } from '../../../server/auth.cf';
...
export const ServerRoute = createServerFileRoute('/api/ai/').methods({
  POST: async ({ request, context }) => {
    // Cast context to our Env type
    const env = context as Env | undefined;

    // Type guard to ensure env is defined
    if (!env) {
      return new Response('Server context not available', { status: 500 });
    }

    // Check if required env properties are available
    if (!env.AUTH_DB || !env.AI_RATELIMIT || !env.QUOTA_DO || !env.AI_GATEWAY_URL) {
      return new Response('Server configuration error', { status: 500 });
    }

    /* ---------- session check ---------- */
    const authApp = buildAuthApp(env);
    // @ts-ignore - auth is added dynamically to the app
    ...
    /* ---------- monthly quota check ---------- */
    // @ts-ignore - DurableObject types are complex
